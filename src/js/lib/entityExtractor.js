...
export function extractEntities(text, conversationContext) {
    ...
    // Fallback for Service if context expects it and fuzzy search failed
    if (conversationContext?.awaitingSlot === 'service' && !allEntities.some(e => e.type === 'service')) {
        // Try exact match first
        const matchedService = ALL_SERVICES.find(s =>
            s.name.en.toLowerCase() === lowerText ||
            s.name.es?.toLowerCase() === lowerText ||
            s.type.toLowerCase() === lowerText ||
            lowerText.includes(s.type.toLowerCase()) ||
            lowerText.includes(s.name.en.toLowerCase()) ||
            (s.name.es && lowerText.includes(s.name.es.toLowerCase()))
        );

        if (matchedService) {
            allEntities.push({ type: 'service', value: matchedService.id, raw: text, confidence: 0.85 });
        } else {
            // Last resort: pass through the text as-is for service selection
            allEntities.push({ type: 'service', value: cleanedText, raw: text, confidence: 0.6 });
        }
    }
